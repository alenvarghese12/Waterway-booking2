...
const BoatViewDetails = () => {
    ...
    const fetchUnavailableDates = async () => {
      try {
        if (boat && boat._id) { // Ensure boat._id is available
          const response = await fetch(`https://waterway-booking2.onrender.com/api/boats/${boat._id}/unavailable-dates`); // Use boat._id instead of boatId
          if (!response.ok) {
    ...
    const options = {
      ...
      handler: async (response) => {
        try {
          ...
          const bookingResponse = await fetch('https://waterway-booking2.onrender.com/api/bookings/bookingss', {
            method: 'POST',
            headers: {
  ...
  useEffect(() => {
    const fetchFoodData = async () => {
      try {
        const response = await axios.get(`https://waterway-booking2.onrender.com/api/food/${boat._id}/food`);
