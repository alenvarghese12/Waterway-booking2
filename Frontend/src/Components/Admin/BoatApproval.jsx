...
const BoatApproval = () => {
  ...
  useEffect(() => {
    ...
    const fetchPendingBoats = async () => {
      try {
        const response = await axios.get('https://waterway-booking2-1.onrender.com/api/boats/pending-boats');
  ...
  const handleApprove = async (boatId) => {
    try {
      await axios.put(`https://waterway-booking2-1.onrender.com/api/boats/approve/${boatId}`);
  ...
  const handleDisapprove = async (boatId) => {
    try {
      await axios.delete(`https://waterway-booking2-1.onrender.com/api /boats/disapprove/${boatId}`);
        ...
        {pendingBoats.length === 0 ? (
          ...
          pendingBoats.map(boat => (
            <div key={boat._id} className="boat-item">
              <img
              src={`https://waterway-booking2-1.onrender.com/uploads/${boat.image}`} // Assuming boat image is saved in an 'uploads' folder on the server
              alt={boat.boatName}
