...
const Addboat = () => {
  ...
  const handleAddOrEditBoat = async (e) => {
    ...
    try {
      if (isEditing) {
        ...
        await axios.put(`https://waterway-booking2.onrender.com/api/boats/${editBoatId}`, {
        ...
        // Add new boat type
        const response = await axios.post('https://waterway-booking2.onrender.com/api/boats', {
          type: trimmedBoatType,
        });
  ...
  const handleDeleteBoat = async (id) => {
    try {
      ...
      await axios.delete(`https://waterway-booking2.onrender.com/api/boats/boatsde/${id}`);
