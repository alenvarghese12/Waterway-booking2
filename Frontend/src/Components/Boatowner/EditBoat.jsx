...
const EditBoat = ({ boat, onUpdate, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (boat.image) {
      setImagePreview(`https://waterway-booking2.onrender.com/uploads/${boat.image}`); // Update this path as needed
    }
  }, [boat]);
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      setImagePreview(`https://waterway-booking2.onrender.com/uploads/${boat.image}`); // Reset to current image if no file selected
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`https://waterway-booking2.onrender.com/api/boats/boatsd/${boat._id}`, formDataToSubmit, {
        headers: {
